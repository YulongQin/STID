...
#' )
#' }
CalNicheComposition <- function(STID_obj = NULL, samp_mode  = "SS",
  ...
  match.arg(samp_mode, .STID_globals$samp_mode)
  if(is.null(group_by)){
...
#' )
#' }
CalNicheAggIndex <- function(STID_obj = NULL,samp_mode  = "SS",
                             loop_id = "LoopAllSamp",
                             samp_grp_index = FALSE,
                             meta_key = NULL,
                             niche_key = NULL,
...
#' )
#' }
CalNicheCoLoc <- function(STID_obj = NULL,
                          loop_id = "LoopAllSamp",
                          niche_key = NULL,
                          meta_key = NULL,
                          group_by = NULL,
...
#' @param interval Spatial scaling intervals
...
.mistyR_pipeline <- function( STID_obj = NULL,
  ...
  for(i in seq_along(loop_single)){
    ...
    misty.views <- misty.intra %>%
      mistyR::add_juxtaview(pos, neighbor.thr = mistyR_params[1]*i_interval) %>%  # add_juxtaview，阈值和半径过大会比较慢
      mistyR::add_paraview(pos, l = mistyR_params[2]*i_interval)
...
#' @param grp_nm Character, group name for output organization (default: NULL)
#' @param dir_nm Character, directory name for output (default: "M3_CalNicheDEGs")
...
#' )
#' }
CalNicheDEGs <- function(STID_obj = NULL, samp_mode  = "SS",
                         loop_id = "LoopAllSamp",
                         samp_grp_index = FALSE,
                         meta_key = NULL,
                         niche_key = NULL,
                         ...
                         remove_genes = NULL,
                         return_data = TRUE,
                         grp_nm = NULL, dir_nm = "M3_CalNicheDEGs",
                         ...
){
  on.exit(while(sink.number() > 0){sink()}, add = TRUE)
...
#' @param return_data Logical, whether to return results list (default: TRUE)
#' @param grp_nm Character, group name for output organization (default: NULL)
#' @param dir_nm Character, directory name for output (default: "M3_CalNicheGeneCor")
#'
...
#' )
#' }
CalNicheGeneCor <- function(STID_obj = NULL,
                            loop_id = "LoopAllSamp",
                            niche_key = NULL,
                            meta_key = NULL,
                            p.features = NULL,
...
#' )
#' }
CalNicheCellComm <- function(STID_obj = NULL,
                             niche_key = NULL,
                             group_by = NULL,
                             assay_id = "Spatial",
                             layer_id = "data",
...
#' )
#' }
CalNicheGRN <- function(STID_obj = NULL,
                        loop_id = "LoopAllSamp",
                        niche_key = NULL,
                        group_by = NULL,
                        ref_data = NULL,
...
#' )
#' }
Plot_DistLine_Exp <- function(STID_obj = NULL, features = NULL, feature_colnm = NULL,
...
#' )
#' }
Plot_DistLine_Ratio <- function(STID_obj = NULL, celltypes = NULL,  group_by = NULL,
                                facet_grpnm = NULL, meta_key = NULL, loop_id = "LoopAllSamp",
                                interval_ratio = NULL ,
                                col = COLOR_List[["PALETTE_WHITE_BG"]], linewidth = 1,ncol = 4
