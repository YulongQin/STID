          ...
          function(seurat_obj = NULL, host_org = NULL, pathogen_grp = NULL, pathogen_org = NULL,
            ...
            if(data_format == "StereoSeq"){
              interval_x <- seurat_obj@meta.data$x %>% unique() %>% sort() %>% diff() %>%
                table() %>% as.data.frame() %>% arrange(desc(Freq)) %>% .[1,1] %>% levels() %>% as.numeric()
...
CreateMultiSampNiche.STID <- function(
  ...
  if(!multi_id %in% valid_multi){
    ...
    if(!is_empty(niche_cells)){
      ...
      STID_niche_info <- STID_obj@STID_analysis@SingleSampNiche[[multi_id]]@niche_info
...
#' Test method for Seurat/STID objects
#'
#' Internal test function for method dispatch demonstration.
#'
#' @param object A Seurat or STID object
...
setMethod(".test", "STID",
          function(object, i) {
            i <- i+5
            callNextMethod()
            cat(paste0("This is an STID object. You provided i = ", i, "\n"))
          }
)
