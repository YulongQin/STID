...
- selecting control samples or spatial background regions for background
  modeling;
- running
...
Background can be estimated from control samples or, when needed, from
spatially defined background regions.
...
[`CorrectBackground()`](https://yulongqin.github.io/STID/reference/CorrectBackground.md)
uses `ctrl_samp_id` for control samples, `bg_region_cell` for predefined
regions, and `bg_region_lasso = TRUE` for interactive lasso selection.
...
(`adjust_UMI = TRUE`) to account for sequencing-depth differences.
Corrected values below zero are set to zero.
...
- Confirm that control samples or selected spatial background regions
  represent true baseline conditions before running correction.
- Use `ctrl_samp_id` for control samples; use `bg_region_cell` or
  `bg_region_lasso = TRUE` when the background must be defined
  spatially.
