# Authors and Citation

## Authors

- **Yulong Qin**. Author, maintainer, copyright holder.
  [](https://orcid.org/0009-0009-2761-0750)

## Citation

Source:
[`DESCRIPTION`](https://github.com/YulongQin/STID/blob/HEAD/DESCRIPTION)

Qin Y (2026). *STID: Spatial Transcriptomics toolkit for Infectious
Diseases*. https://github.com/YulongQin/STID.git,
https://yulongqin.github.io/STID/, https://github.com/YulongQin/STID.

    @Manual{,
      title = {STID: Spatial Transcriptomics toolkit for Infectious Diseases},
      author = {Yulong Qin},
      year = {2026},
      note = {https://github.com/YulongQin/STID.git,
        https://yulongqin.github.io/STID/,
        https://github.com/YulongQin/STID},
    }
